...
2026-06-01 lint → первый проход vault'а: переименован `entities/competitors/index.md` → `entities/competitors.md` (резолвит ссылки `[[competitors]]` в index/content-strategy/log/0001-warmup-content); починен `[[entities/competitors]]` → `[[competitors]]` в архивной 0001-warmup-content; в index `[[trainer-blogger]]` помечен как «планируется» (страницы нет, чтобы не путать с broken-link). Outdated/противоречий не нашёл — vault молодой (<14 дней). (Steve)
