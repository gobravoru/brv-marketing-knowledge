...
2026-05-28 lint → первый прогон vault: переименовал `entities/competitors/index.md` → `entities/competitors.md` (3 битые ссылки `[[competitors]]` теперь резолвятся), убрал из index.md `[[trainer-blogger]]` (страницы нет), разблочил `[[dzen]]`/`[[seo]]` (планируемые, ещё не страницы), починил `[[capture-knowledge]]` в [[learnings]] (skill, не wiki). Орфанов нет, frontmatter всех `active`-страниц свежее 7 дней. (Steve)
