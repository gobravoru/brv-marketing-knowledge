...
2026-06-06 lint → первый проход по vault'у: переименовал `entities/competitors/index.md` → `entities/competitors.md` (basename-коллизия с `wiki/index.md`, [[competitors]] резолвится чисто); убрал broken `[[trainer-blogger]]` из index (страницы нет, для 0→1 не приоритет); расплейсхолдерил `[[dzen]] / [[seo]] / [[posts/]] / [[raw/interviews/...]]` в index/hook-formulas/proof-points — теперь обычный текст вместо false-broken wikilinks. Hard contracts не трогал, противоречий не нашёл, orphan'ов нет, всё active обновлено <60 дней. (Steve)
