...
2026-06-05 lint → fixed 2 broken/unstable links ([[competitors]] резолвится после переноса `entities/competitors/index.md` → `entities/competitors.md`; [[capture-knowledge]] в [[learnings]] заменён на ссылку на skill в `.claude/skills/`, скилл живёт вне vault'а), resolved 1 orphan ([[content-registry]] подключён из [[content-strategy]] — раздел «Антиповтор» + «Связано»), flagged 0 outdated pages (все wiki обновлены ≤14 дней). Также убран wikilink на несуществующий [[trainer-blogger]] в index — оставлен как плейсхолдер курсивом до первой работы с сегментом. [[goals]] остаётся «orphan beyond index» осознанно — это apex-страница, входная по дизайну. PR (не мёрж). (Steve)
