...
2026-06-08 lint → первый проход vault'а: починены broken-wikilinks (`[[competitors]]` теперь резолвится — `wiki/entities/competitors/index.md` → `wiki/entities/competitors.md`; убраны фиктивные `[[posts/]]`, `[[raw/interviews/...]]`, `[[capture-knowledge]]` → заменены на код/markdown; `[[trainer-blogger]]`/`[[dzen]]`/`[[seo]]` в index переведены в обычный текст до появления страниц). Orphan `goals` — корневой state-файл, оставлен. Technical-debt: в `proof-points` остаются `[[bravo-product]]`/`[[raw/product-docs/target-audience]]` — по конвенции vault'а на raw нужно ссылаться markdown'ом, поправить отдельным проходом. PR — без мёржа, ждёт Артёма (Steve)
