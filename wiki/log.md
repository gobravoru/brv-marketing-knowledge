...
2026-06-08 lint → fixed 4 broken links: [[bravo-product]] → [[raw/product-docs/bravo-product]] (proof-points ×2, learnings ×1), [[entities/competitors]] → [[competitors]] (0001-warmup-content). Переименован `entities/competitors/index.md` → `entities/competitors.md` (basename-коллизия с `wiki/index.md` в Obsidian — `[[competitors]]` теперь резолвится). Flagged for Артёма: `[[trainer-blogger]]` в index.md висит без страницы — сегмент не описан, решить нужен ли. Orphans не нашёл. PR-only, не мёрж. (Steve)
