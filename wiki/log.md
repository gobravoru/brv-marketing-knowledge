...
2026-06-02 lint → 1-й прогон vault'а. Broken link 1: `[[trainer-blogger]]` в index.md (страница не заведена) → размечен как «планируется», по аналогии с [[dzen]]/[[seo]]. Orphans: нет (все wiki-страницы, кроме [[goals]], имеют входящие ссылки; goals — top-level, ссылается из index и `.claude/CLAUDE.md`). Frontmatter alert: hard contracts [[positioning]], [[brand-voice]], [[banned-words]] всё ещё `status: draft` с bootstrap'а — нужен approve Артёма на флип `draft → active` (флаг, сам не правлю). Soft refs `[[capture-knowledge]]` (skill вне vault) и `[[raw/interviews/...]]` (placeholder) — оставлены как есть. (Steve)
